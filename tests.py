...
def run_prob_test_graph(stream, truth, num_samples):
    samples = []
    ...
    # print("np-samples", np.array(samples))
    # print("truth_dist.cdf", truth_dist.cdf, type(truth_dist.cdf))
...
def run_prob_test(stream, truth, num_samples):
    samples = []
    for i in range(int(num_samples)):
        # had to modify this line for by adding [0] for evaluation based sampling.  
